...
public class UnstableZmejCoinEffect {
    ...
    public void onWorldTick(TickEvent.PlayerTickEvent event) {
        ...
        if (ticks >= 200) { // 10 seconds
            ...
            convertSingleZmejCoin(player);
        }
    }

    // Helper method to check if the player has an Unstable Jemz Coin
    private boolean hasUnstableZmejCoin(Player player) {
        for (ItemStack stack : player.getInventory().items) {
            if (stack.getItem() instanceof ZmejcoinUnstableItem) {
    ...
    private void convertSingleZmejCoin(Player player) {
        if (hasUnstableZmejCoin(player)) {
            for (int i = 0; i < player.getInventory().getContainerSize(); i++) {
                ItemStack stack = player.getInventory().getItem(i);
                if (stack.getItem() == ModItems.ZMEJCOIN.get() && stack.getCount() > 0) {
                    stack.shrink(1); // Remove one JemzCoin
                    player.getInventory().add(new ItemStack(ModItems.JEMZCOIN.get(), 1)); // Add one Zmejcoin
